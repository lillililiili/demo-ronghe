    ...
    <header class="detail-head">
      ...
      <section v-else ref="detailBody" class="risk-detail" aria-label="空域记录详情">
        <div class="detail-hero detail-hero-micro"><div class="detail-hero-inner">
          <div class="detail-hero-icon" v-html="targetIcon(row.target)"></div>
        ...
        <section class="sect"><h4>监测信息</h4><dl class="kv kv-surface">
          ...
          <template v-if="row.target">
            ...
            <dt>最近监测</dt><dd>{{ time(row.target.last_seen_at) }}</dd>
            <template v-if="!row.target.point"><dt>定位状态</dt><dd>位置未记录，无法定位</dd></template>
            <dt>监测来源</dt><dd>{{ labelOf(SOURCE_MODE_LABEL, row.target.source_mode, '未记录') }}</dd>
            <dt v-if="row.target.risk_summary">历史风险时间</dt><dd v-if="row.target.risk_summary">{{ time(row.target.risk_summary.occurred_at) }}</dd>
            <dt v-if="!row.risk">风险记录</dt><dd v-if="!row.risk">{{ row.target.demo ? '模拟观测，无业务风险记录' : row.target.risk_summary ? '仅有摘要，完整记录未读取' : '未查到关联记录，风险待确认' }}</dd>
          </template>
        </dl>
          <details v-if="row.target && (row.target.point || row.target.demo)" :key="row.key" class="monitor-basis"><summary>查看监测依据</summary><dl class="kv">
            <dt>坐标</dt><dd>{{ pointText(row.target.point) }}</dd>
            <template v-if="row.target.demo"><dt>前帧距离</dt><dd>{{ Math.round(row.target.demo.previousDistance) }} 米</dd></template>
          </dl></details>
        </section>
...
.monitor-basis { margin-top: 12px; font-size: 12px; }
.monitor-basis summary { cursor: pointer; color: var(--txt-2); padding: 6px 0; }
.monitor-basis summary:focus-visible { outline: 2px solid var(--cyan); outline-offset: 2px; }
