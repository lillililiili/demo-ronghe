...
const rows = ref([]), selected = ref(null), page = ref(1), total = ref(0), status = ref('REQUESTED');
...
async function refresh(id) {
  ...
  try {
    ...
    if (active && seq === detailRequest) { selected.value = detail; await load(); }
    return detail;
  } catch (e) {
...
function show(id) { refresh(id).catch(() => {}); }
function resize(size) { pageSize.value = size; load(1); }
function act(action, row) { selected.value = row; action.open({ authorization: row, refresh: () => refresh(row.authorization_id) }); }
onMounted(() => load());
onUnmounted(() => { active = false; request++; });
</script>

<template>
  <UPanel title="处置授权" sub="先申请、由另一人审批、执行并核对设备回执，再提交处罚交接" panel-style="flex:1;min-height:0;overflow:auto">
    <div class="toolbar">
      ...
      <button class="btn" :disabled="loading" @click="selected ? show(selected.authorization_id) : load()">刷新状态</button>
    </div>
      ...
      <table class="tb">
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.authorization_id">
            <td><button class="btn mono" :title="row.authorization_id" @click="show(row.authorization_id)">{{ row.authorization_no }}</button></td>
    ...
    <section v-if="selected" class="sect" aria-label="所选授权详情">
      <h4>{{ selected.authorization_no }} · {{ disposalStatusText(selected) }}</h4>
      <p>{{ selected.reason }}</p>
