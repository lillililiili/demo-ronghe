...
async function main() {
  ...
  const targetRow = {
    target_id: 't1', target_no: 'MB-1', object_type_code: 'UAV', source_mode: 'replay', last_seen_at: now,
  ...
  const snapshots = [];
  const errors = [];
  const source = createSituationApiSource({ fastMs: 10, slowMs: 10_000, now: () => now + targetCalls * 1000 });
