...
<template>
  <section class="auto-sms-notice" aria-label="飞手短信通知" :data-state="data?.auto_sms?.status">
    <header><b>飞手短信</b><span v-if="view.simulated" class="tag t-amber">模拟短信</span></header>
    <p class="asn-title" :class="`asn-${view.tone}`">{{ view.title }}</p>
    ...
    <p v-if="view.source === 'RULE_ILLEGAL'">触发依据来自系统研判，无需等待人工核实后再通知。</p>
