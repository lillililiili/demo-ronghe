...
class DisposalExecutionGatewayTest {
    ...
    private DisposalExecutionGateway gateway(boolean bound, boolean enabled, boolean online) {
        ...
        when(control.enqueue(anyString(), anyString(), anyString(), anyInt(), anyInt(), any(), anyString()))
                .thenReturn("cmd-1");
        return new DisposalExecutionGateway(control, mqtt, devices);
    }
    ...
    @Test
    void fourChannelIsRejectedWithoutCallingA() {
        DisposalExecutionGateway.Rejected rejected = DisposalExecutionGateway.noCapability("COUNTERMEASURE_4CH");
