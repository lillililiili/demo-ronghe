...
2026-09-17 最新页面约定：按用户截图删除“需要你处理”“补充操作与事后移送”及“飞手通知与现场记录”整块，移除告警页手动移送与现场补充入口。短信、电话各自结果和现场、授权摘要保留，后台历史及冻结材料不删除。以下旧章节中的人工操作与历史展开仅描述当时实现，不再是当前告警页入口。

处罚移送改为后端自动办理的方向已确认，前端只读取 `advisory.auto_handoff` 并按实际 `handoff_id` 进入同一处罚交接。等待、阻断、未启用、失败与已有交接分别显示；旧服务未提供字段时显示“自动移送状态暂不可用”，不把配置缺失显示为成功。后端配套尚未实现，当前共享服务尚未启用自动移送，本轮页面代码变化不代表已完成后台或真实通道验收；具体待批准范围见[后台配套说明](automatic-punishment-handoff-review.md)。
...
告警事件详情：核实属实后才自动发送短信（2026-09-22 用户确认）。短信送达后观察 10 秒，仍在告警空域才打电话；打完再观察 10 秒，仍未撤离则状态为待反制并显示发起反制。没有已核验执行飞手时，核实后直接待反制。待核实期间不发短信。已离开或无法确认不打电话、不进入待反制。页面先展示通知结果、异常原因与数据时间；失败且后台允许时提供“重新发送通知”。常规流程不再要求点击“短信劝离”。电话录音、反制申请、授权查看与独立移送仍按各自条件办理，自动短信不会替代反制授权或处罚决定。联系与观察历史继续保留。
...
| --- | --- |
| 触发数据 | 已建立的无人机告警事件 |
| 自动依据 | 事件建立后即发送。核实结论、合法性研判、目标类型和观测时效不再作为门槛。没有已核验的执行飞手时不发送 |
| 时效 | 不再用 120 秒或 300 秒拦截短信。下表之前的时效只保留给电话录音和反制等其他动作 |
| 自动动作 | 后台约 10 秒检查一次；每条事件自动发送一条模拟短信，稳定渠道幂等键，不依赖浏览器 |
| 人工介入 | 发送失败后可带说明重试。已送达的记录不重复自动发送 |
| 失败处理 | 发送中、失败、未接通与模拟送达分开记录；页面读取不发送，失败不冒充送达 |
| 来源边界 | 策略仅在 local/test 且已启用时运行。这些环境里 mock、replay、live 都记模拟送达。生产环境不发送 |
| 接收对象 | 只发给已核验的执行飞手。没有飞手、联系方式未核验或名册不可用时，短信和电话都不发送，不使用演示飞手或单位联系人 |
