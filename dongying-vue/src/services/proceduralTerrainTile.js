...
// 主线程兼容路径与 Worker 共用同一像素算法，避免改变现有底图质感。
export async function renderTerrainTile(url) {
  const canvas = typeof OffscreenCanvas === 'function'
    ? new OffscreenCanvas(TILE, TILE) : document.createElement('canvas');
  canvas.width = canvas.height = TILE;
  const m = /^procdem:\/\/(\d+)\/(\d+)\/(\d+)/.exec(url || '');
  try {
    if (m) encode(canvas, Number(m[1]), Number(m[2]), Number(m[3]));
    else { const ctx = canvas.getContext('2d', { willReadFrequently: true }); ctx.fillStyle = 'rgb(128,0,0)'; ctx.fillRect(0, 0, TILE, TILE); }
  } catch { /* 沿用原有的装饰层降级。 */ }
  const blob = typeof canvas.convertToBlob === 'function'
    ? await canvas.convertToBlob({ type: 'image/png' })
    : await new Promise(resolve => canvas.toBlob(resolve, 'image/png'));
  if (!blob) throw new Error('地图纹理编码失败');
