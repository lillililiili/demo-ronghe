...
> 状态：v1.2（2026-09-07）。A 侧 P1 协议 A MQTT 信封与 P3 协议 C 光电边端信封已按实现冻结；B 侧阶段 8.5 领取前缀、映射入口与 `ingest_seq` 已落地（§6–§7）。P4-A 提升默认关、P5 控制仍待冻结。依据：会议纪要 V1.0、凌云协议 A v8.6 / B V2.4 / C 20250826、决策 8-30、`target-schema-v1-alignment.md` §5、决策 8.5-12/20–29。配套计划：《协作者 A 直连接入计划》《协作者 B 直连切片计划》。
