...
<template>
  <ol class="advisory-records">
    <li v-for="row in rows" :key="row.record_id">
      <div class="ar-heading"><b>{{ ADVISORY_KIND[row.kind] || '处置记录' }}</b><span v-if="row.simulated" class="tag t-amber">模拟</span></div>
      ...
      <p v-else>{{ row.recipient_name || '接收主体未提供' }}<span v-if="row.kind === 'SMS_SIMULATED'"> · {{ row.delivery_status === 'SIMULATED_DELIVERED' ? '模拟送达' : '模拟发送记录' }}，未发送真实短信</span></p>
...
<style scoped>
.advisory-records{list-style:none;margin:0;padding:0}.advisory-records li{padding:12px 0;border-top:1px solid var(--line);overflow-wrap:anywhere}.ar-heading{display:flex;gap:8px;align-items:center}.advisory-records p{margin:6px 0;line-height:1.6;font-size:13px}.advisory-records small,.ar-muted{color:var(--muted);font-size:12px}.ar-heading b{font-size:13px}
</style>
