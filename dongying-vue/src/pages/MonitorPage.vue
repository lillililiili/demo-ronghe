...
const linkedDevice = ref(null);
const requestedDeviceId = ref('');
const deviceLinkError = ref('');
...
const selected = computed(() => tree.value.find(item => item.device_id === selectedId.value)
  || (linkedDevice.value?.device_id === selectedId.value ? linkedDevice.value : null));
...
const metricOptions = computed(() => optionsOf((state.value?.metrics || []).map(item => [item.code, `${metricLabel(item)}${item.unit ? `（${item.unit}）` : ''}`])));
...
const statusText = value => ({ ONLINE: '在线', OFFLINE: '离线', ABNORMAL: '异常', UNKNOWN: '未知', CONNECTED: '已连接', DISCONNECTED: '未连接', CONNECTING: '连接中', DEGRADED: '通信质量下降' })[value] || '未知';
/* 指标编码与开关量都是协议里的英文名，上屏说人话。 */
const METRIC_LABEL = { link_latency_ms: '通信延迟', packet_loss_pct: '丢包率', packet_loss_rate: '丢包率', signal_dbm: '信号强度', signal_strength_dbm: '信号强度', relay_state: '继电器状态字', temperature_c: '温度', cpu_pct: '处理器占用', memory_pct: '内存占用' };
const metricLabel = metric => metric?.label || METRIC_LABEL[metric?.code] || '指标';
const metricValue = value => (value === 'ON' ? '开' : value === 'OFF' ? '关' : value ?? '—');
const COMMAND_STATUS_LABEL = { QUEUED: '已排队', SENT: '已下发', SUCCEEDED: '已成功', FAILED: '失败', TIMEOUT: '超时', CANCELLED: '已取消' };
const statusType = value => ({ ONLINE: 'success', OFFLINE: 'default', ABNORMAL: 'error', UNKNOWN: 'warning' })[value] || 'default';
/* 指标来源只给内部适配器名；屏幕上说人话，认不出的就说“来源未说明”，不把英文名漏出去。 */
const METRIC_SOURCE_TEXT = { 'mock-adapter': '模拟设备数据', 'device-report': '设备上报', 'radar-direct': '雷达直连', 'mqtt': '消息接入' };
function metricSourceText(source) { if (!source) return '来源未说明'; return METRIC_SOURCE_TEXT[source] || (/[\u4e00-\u9fff]/.test(source) ? source : '来源未说明'); }
...
async function loadAggregate(showBusy = false) {
  ...
  try {
    ...
    if (requestedDeviceId.value) {
      ...
      // 树最多显示 500 台；指定设备不在这一批时，仍按 ID 读取，不能误选第一台。
      ...
      if (requested) {
        ...
        collapsedGroups.value = { ...collapsedGroups.value, [requested.channel || '未分组']: false };
      } else {
        selectedId.value = '';
        deviceLinkError.value = '未找到指定设备，设备可能已移除或不在你的查看范围内。';
      }
    } else if (!selected.value) selectedId.value = tree.value[0]?.device_id || '';
    error.value = '';
...
async function loadSelected(showBusy = false) {
  ...
  try {
    const [stateData, historyData, eventData, protocolData, targetData, deviceIncidentData] = await Promise.all([
      ...
      deviceApi.incidents({ device_id: requestDeviceId, page: 1, size: 20 })
    ]);
    if (disposed || requestDeviceId !== selectedId.value || requestMetricCode !== metricCode.value) return;
    ...
    selectedIncidents.value = deviceIncidentData.items;
  ...
  } catch (e) { if (!disposed && requestDeviceId === selectedId.value) error.value = e.message || '所选设备状态加载失败'; }
  finally {
...
function selectDevice(item) {
  requestedDeviceId.value = ''; deviceLinkError.value = '';
  if (linkedDevice.value?.device_id !== item.device_id) linkedDevice.value = null;
  selectedId.value = item.device_id;
}

function applyFilters() { loadAggregate(true).then(() => loadSelected(true)); }

function syncDeviceLink() {
  if ((location.hash || '').split('?')[0] !== '#/monitor') return;
  const id = new URLSearchParams((location.hash || '').split('?')[1] || '').get('device');
  if (!id) return;
  requestedDeviceId.value = id; deviceLinkError.value = '';
  linkedDevice.value = null;
  selectedId.value = '';
  Object.assign(filters, { keyword: '', channel: null, type_code: null });
  void loadAggregate(true);
}

watch(selectedId, () => {
  state.value = null; protocolStatus.value = null; radarTargets.value = []; selectedIncidents.value = [];
  history.value = []; events.value = []; eventSeq.value = 0; command.value = null;
  if (selectedId.value) loadSelected(true);
}, { flush: 'sync' });
watch(metricCode, () => { history.value = []; loadSelected(true); });
onMounted(async () => {
  window.addEventListener('hashchange', syncDeviceLink);
  syncDeviceLink();
...
onUnmounted(() => { disposed = true; window.removeEventListener('hashchange', syncDeviceLink); clearInterval(aggregateTimer); clearInterval(selectedTimer); window.CH.disposeEl(chartEl.value); });
</script>
...
<template>
  <div class="view monitor-page" id="view" :aria-busy="loading || selectedLoading">
    ...
    <div v-if="deviceLinkError" class="warnbox" role="alert">{{ deviceLinkError }}</div>
    <div class="monitor-grid">
      ...
      <div class="center-column">
        <UPanel title="实时状态" :sub="selected ? selected.device_no : '未选择设备'">
          <p v-if="selected" class="selected-device-name">{{ selected.name }}</p>
          <NSpin :show="selectedLoading"><template v-if="state"><div class="state-hero"><div><small>连接状态</small><strong>{{ statusText(state.connectivity) }}</strong></div><div><small>健康状态</small><strong>{{ healthText(state.health_code) }}</strong></div><div><small>最近在线上报</small><strong class="time-value">{{ fmtTime(state.last_heartbeat_at) }}</strong></div></div><div v-if="state.connectivity === 'OFFLINE'" class="metric-offline">设备已离线，下方曲线显示离线前的数据</div><div v-else class="metric-grid"><article v-for="metric in state.metrics" :key="metric.code"><small>{{ metricLabel(metric) }}</small><b>{{ metricValue(metric.value) }} <em>{{ metric.unit || '' }}</em></b><span>{{ metricSourceText(metric.source) }}</span></article></div><div v-if="protocolStatus?.protocol_code" class="protocol-state"><header><b>{{ protocolLabel(protocolStatus) }}</b><NTag size="small" :type="protocolStatus.connection_state==='ONLINE'?'success':'warning'" :bordered="false">{{ statusText(protocolStatus.connection_state) }}</NTag></header><p v-if="protocolStatus.blocking_reason">{{ protocolStatus.blocking_reason }}</p></div><div v-if="command" class="command-card"><div><b>{{ command.command_no }}</b><span>{{ command.simulated ? '模拟回执' : '设备回执' }} · {{ COMMAND_STATUS_LABEL[command.status] || '处理中' }}</span></div><p>{{ command.result_detail || '指令已排队，等待后台处理。' }}</p></div></template><NEmpty v-else description="请选择设备查看状态" /></NSpin>
        </UPanel>
        <UPanel v-if="selected?.protocol_code==='RADAR_TCP_V3_0_0'" title="最近活动航迹" sub="这里显示雷达测得的目标类型和位置，是否存在风险或违规需另行判断" nopad><NDataTable v-if="radarTargets.length" :columns="targetColumns" :data="radarTargets" :row-key="row=>row.target_id" :scroll-x="660" :max-height="210" /><NEmpty v-else description="当前没有活动航迹；雷达待机时保持无数据" class="empty-block" /></UPanel>
      ...
      <div class="right-column">
        <UPanel v-if="selectedId" title="所选设备告警" sub="最近 20 条" nopad>
          <div v-if="selectedIncidents.length" class="selected-incident-list">
            <article v-for="item in selectedIncidents" :key="item.incident_id">
              <NTag size="small" :type="item.closed_at ? 'default' : 'error'" :bordered="false">{{ item.closed_at ? '已关闭' : '尚未关闭' }}</NTag>
              <p>{{ item.reason }}</p>
              <time>发现时间：{{ fmtTime(item.detected_at) }}</time>
              <time v-if="item.closed_at">关闭时间：{{ fmtTime(item.closed_at) }}</time>
            </article>
          </div>
          <NEmpty v-else-if="!selectedLoading" description="尚无告警记录，请结合实时状态查看设备异常" class="empty-block" />
        </UPanel>
        <UPanel title="活动告警（全部设备）" :sub="`${incidents.length} 条 · 点击定位设备`" nopad><div v-if="incidents.length" class="feed"><button v-for="item in incidents" :key="item.incident_id" type="button" @click="selectDevice(item)"><span><NTag size="small" :type="item.severity==='HIGH'?'error':'warning'" :bordered="false">{{ labelOf(SEVERITY_LABEL, item.severity) }}</NTag><b>{{ item.device_name }}</b></span><p>{{ item.reason }}</p><time>{{ fmtTime(item.detected_at) }}</time></button></div><NEmpty v-else description="当前没有活动告警" class="empty-block" /></UPanel>
...
.selected-device-name { margin: 0 0 12px; font-weight: 600; color: var(--txt-1); }
.selected-incident-list { max-height: 230px; overflow: auto; }
.selected-incident-list article { display: grid; gap: 6px; padding: 12px; border-bottom: 1px solid var(--line-1); }
.selected-incident-list .n-tag { justify-self: start; }
.selected-incident-list p { margin: 0; color: var(--txt-2); line-height: 1.5; overflow-wrap: anywhere; }
.selected-incident-list time { color: var(--txt-3); font-size: 11px; }
...
.metric-offline{padding:14px 12px;color:#8aa0b8;font-size:12px;line-height:1.7}
</style>
