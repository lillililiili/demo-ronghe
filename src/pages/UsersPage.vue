<script>
/* 模块级状态：跨导航保持（legacy 约定 —— 页签/筛选/选中/权限草稿在切页后不丢）。
   放在非 setup 的模块作用域，组件重挂不会重置。 */
const state = { tab: 'user', sel: null, kw: '', permDraft: null };
export default {};
</script>

<script setup>
/* 用户与权限 —— 第二个转换页（源：legacy pages/users.js）。
...
const tab = ref(state.tab);
const kw = ref(state.kw);
const bump = ref(0);              // permDraft 是深结构，改动后 bump++ 触发重算

/* ---------- 用户管理（函数体与 legacy 同构） ---------- */
function userTab() {
  state.sel = state.sel || M.users[0];
  const rows = M.users.filter(u => !state.kw || u.name.includes(state.kw) || u.account.includes(state.kw) || u.org.includes(state.kw));
  ...
  ], rows, { rowId: u => u.id, activeId: state.sel && state.sel.id })}
  </div>
...
/* ---------- 角色与权限矩阵（与 legacy 同构；真值只有 M.PERM 一份） ---------- */
...
const permVal = (rid, i) => (state.permDraft ? state.permDraft[rid] : M.PERM[rid])[i];
function permChanges() {
  if (!state.permDraft) return [];
  const out = [];
  permRows().forEach(x => M.ROLES.forEach(r => {
    const from = M.PERM[r.id][x.i], to = state.permDraft[r.id][x.i];
...
function roleTab() {
  ...
  return `<div style="flex:1;display:flex;flex-direction:column;min-width:0;padding:12px;overflow:auto">
    ...
    const v = permVal(r.id, x.i), dirty = state.permDraft && M.PERM[r.id][x.i] !== v;
...
const bodyHtml = computed(() => {
  bump.value; kw.value;
  return tab.value === 'user' ? userTab() : tab.value === 'role' ? roleTab() : auditTab();
});
const toolsHtml = computed(() => {
  bump.value;
  if (tab.value === 'user') return `<input class="ip" id="usKw" style="width:180px" placeholder="搜索姓名 / 账号 / 单位" value="${state.kw}">
    <button class="btn pri" id="usAdd">${U.icon('plus')} 新增用户</button>`;
  if (tab.value === 'audit') return `${U.select('am', ['全部模块', '处置处罚管理', '设备接入调测', '系统登录'])}
...
const TABS = [['user', '用户管理'], ['role', '角色与权限'], ['audit', '操作审计']];
function setTab(k) { state.tab = k; tab.value = k; }

/* 改动重算并保住 .scroll 滚动位置（legacy 约定：草稿单元格点击/搜索不回顶） */
async function repaintKeepScroll() {
  const d = document.getElementById('usBody');
  const sc = d && d.querySelector('.scroll') ? d.querySelector('.scroll').scrollTop : 0;
  bump.value++;
  await nextTick();
...
/* ---------- 事件委托（与 legacy mount 同构） ---------- */
function onClick(e) {
  const t = e.target;
  const row = t.closest('[data-row]');
  const pc = t.closest('[data-pc]');
  const pl = t.closest('[data-pl]');
  const uop = t.closest('[data-uop]');
  if (uop) {
    e.stopPropagation();
    const [op, id] = uop.dataset.uop.split('|');
  ...
  if (row && tab.value === 'user') {
    state.sel = M.users.find(u => u.id === row.dataset.row) || state.sel;
    U.selectRow(document.getElementById('view'), row.dataset.row);   // 只切换选中态,列表不重建
    document.getElementById('usDetail').innerHTML = userDetail();
    return;
  }
  if (pc) {
    const [rid, i] = pc.dataset.pc.split('|');
    if (!state.permDraft) { state.permDraft = {}; M.ROLES.forEach(r => { state.permDraft[r.id] = M.PERM[r.id].slice(); }); }
    const cur = state.permDraft[rid][+i];
    state.permDraft[rid][+i] = PERM_ORDER[(PERM_ORDER.indexOf(cur) + 1) % PERM_ORDER.length];
    repaintKeepScroll();
    return;
  }
  if (pl) {
...
function onInput(e) {
  const kwEl = e.target.closest('#usKw');
  if (!kwEl) return;
  state.kw = kwEl.value.trim();
  const d = document.getElementById('usBody');
  const st0 = d.querySelector('.scroll') ? d.querySelector('.scroll').scrollTop : 0;
  kw.value = state.kw;
  nextTick().then(() => {
    const sc = document.getElementById('usBody')?.querySelector('.scroll');
    if (sc) sc.scrollTop = st0;
    /* v-html 重绘会重建输入框（value 来自 state.kw），焦点保持 */
    const el = document.getElementById('usKw');
...
<template>
  <div class="view" id="view" @click="onClick" @input="onInput">
    <div class="panel" style="flex:1;min-height:0;margin-top:12px;margin-bottom:12px;height:calc(100vh - 138px)">
      ...
      <div class="pb nopad"><div id="usBody" style="flex:1;display:flex;min-height:0" v-html="bodyHtml"></div></div>
    </div>
  </div>
</template>
