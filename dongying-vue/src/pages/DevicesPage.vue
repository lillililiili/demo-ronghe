...
import { MQTT_CONNECTION_LABEL, MQTT_DIAGNOSTIC_OUTCOME_LABEL, MQTT_DIAGNOSTIC_REASON_LABEL, labelOf } from '@/ui/labels.js';
            ...
            <template v-if="selected">
              ...
              <dl class="detail-grid">
                ...
                <template v-if="isMqtt(detail)">
                  ...
                  <dt>MQTT 连接</dt><dd>{{ labelOf(MQTT_CONNECTION_LABEL, protocolStatus?.connection_state || 'DISCONNECTED') }}</dd>
              ...
              <div v-if="isMqtt(detail)" class="connection-card"><b>数据接收情况</b><small v-if="protocolStatus?.blocking_reason">{{ protocolStatus.blocking_reason }}</small><small v-for="(diagnostic, index) in (protocolStatus?.details?.recent_diagnostics || []).slice(0, 5)" :key="index">{{ fmtTime(diagnostic.received_at) }} · {{ labelOf(MQTT_DIAGNOSTIC_OUTCOME_LABEL, diagnostic.outcome) }} · {{ labelOf(MQTT_DIAGNOSTIC_REASON_LABEL, diagnostic.reason) }}</small><small>设备每次上报运行状态后记为在线，连续 30 秒没有上报则记为离线。目标数据收到后由系统处理；支持按设备协议控制诱骗、干扰和驱鸟炮。</small></div>
              <div v-if="isEo(detail)" class="connection-card"><b>光电设备连接情况</b><small v-if="protocolStatus?.details?.camera_status_json">相机状态 {{ protocolStatus.details.camera_status_json }}</small><small v-for="(diagnostic, index) in (protocolStatus?.details?.recent_diagnostics || []).slice(0, 5)" :key="index">{{ fmtTime(diagnostic.received_at) }} · {{ labelOf(MQTT_DIAGNOSTIC_OUTCOME_LABEL, diagnostic.outcome) }} · {{ labelOf(MQTT_DIAGNOSTIC_REASON_LABEL, diagnostic.reason) }}</small><small>根据设备定期上报的状态判断是否在线，跟踪数据收到后由系统处理。当前设备尚不支持角度调整、回到初始位置和辅助识别。</small></div>
              <div v-else-if="selected.source_mode==='live'" class="connection-card protocol-card"><b>协议握手状态</b><NTag size="small" :type="protocolStatus?.connection_state==='ONLINE'?'success':protocolStatus?.connection_state==='ERROR'?'error':'warning'" :bordered="false">{{ labelOf(MQTT_CONNECTION_LABEL, protocolStatus?.connection_state || 'DISCONNECTED') }}</NTag><small>{{ protocolStatus?.blocking_reason || '正在等待系统连接设备' }}</small></div>
            </template>
