-- 阶段 13 验收夹具（仅本地/验收库，勿在生产执行）：
-- 给"两人规则"造一个独立的审批人角色。角色矩阵接口目前只接受 MODULE 码，ACTION 码（disposal:*）没有授予入口，
...
-- 只授处置域的动作权限；模块菜单（alarm 等）请在角色矩阵页里按需勾选。
...
FROM unnest(ARRAY['disposal:read', 'disposal:approve', 'disposal:execute', 'disposal:stop']) AS c
