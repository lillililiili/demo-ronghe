...
<template>
  <div class="airspace-object-markers">
    <button v-for="marker in markers" :key="`${marker.kind}:${marker.id}`" class="object-marker" :class="{ active: marker.active }" type="button"
      ...
      <span class="object-symbol"><span v-html="targetIcon(marker)"></span></span><span class="object-risk-state" aria-hidden="true" v-html="statusIcon"></span>
...
.object-symbol :deep(svg) { width:36px;height:36px; }.object-symbol span { display: flex; }
