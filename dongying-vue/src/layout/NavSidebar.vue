  ...
  <nav class="nav" id="nav" :class="{ mini: store.navMini }">
    <template v-for="n in visibleNav" :key="n.t">
      ...
      <div v-else class="g1" :class="{ open: store.openGrp === n.t, has: n.kids.some(c => c.k === cur) }">
        <div class="l1 gh" :data-grp="n.t" :title="store.navMini ? n.t : undefined" :aria-label="n.t" @click="toggleGrp(n.t)"><i v-html="icon(n.icon)"></i><span>{{ n.t }}</span><b class="ca">›</b></div>
        <div class="l2"><a v-for="c in n.kids" :key="c.k" :class="{ on: cur === c.k }" :href="'#/' + c.k" :data-k="c.k"
          :title="store.navMini ? c.t : undefined" :aria-label="c.t"><em></em><span>{{ c.t }}</span></a></div>
      </div>
    </template>
    <div class="navfoot">
