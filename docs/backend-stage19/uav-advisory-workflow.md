...
告警事件详情：核实属实后才自动发送短信（2026-09-22 用户确认）。短信送达后观察 3 秒，仍在告警空域才打电话；打完再观察 10 秒，仍未撤离则状态为待反制并显示发起反制。没有已核验执行飞手时，核实后直接待反制。待核实期间不发短信。已离开或无法确认不打电话、不进入待反制。页面先展示通知结果、异常原因与数据时间；失败且后台允许时提供“重新发送通知”。常规流程不再要求点击“短信劝离”。电话录音、反制申请、授权查看与独立移送仍按各自条件办理，自动短信不会替代反制授权或处罚决定。联系与观察历史继续保留。
