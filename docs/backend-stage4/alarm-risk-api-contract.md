...
2026-09-14 风险列表范围修正：`GET /risks` 与 `GET /risks/export.csv` 增加可选 `risk_types`，单个字符串以英文逗号分隔，最多 10 项，每项去首尾空白后为 1–64 字符，重复项去重；空项、重复参数或超限返回 `400 VALIDATION_ERROR`。省略时保持原查询范围；与 `risk_type` 同时传入时取交集。筛选在数据库分页及计数之前生效，导出沿用相同条件和权限。飞行页面的“全部风险事件”列表及其状态/等级统计使用 `SPACE_OBJECT,FOREIGN_OBJECT,WEATHER`；其他页面及计划详情的既有查询不变，不迁移或删除数据。代码已调整，按用户要求未构建、未测试，运行效果未验收。

2026-09-14 气象风险补充：`risk_type=WEATHER` 表示气象风险，沿用 `/risks` 列表、详情、核验与通知能力；类型筛选和 CSV 导出使用相同口径，中文名称为“气象风险”。飞行计划风险与空域管理风险共用该类型，计划详情读取当前时间窗口内的各类关联风险。气象依据使用已保存的 `reason_code/reason_text`、等级、时间及来源，不要求关联感知目标或异物空间事实；无可信位置时仅在区县风险列表展示，不匹配所选空域范围。本次未接入气象数据源、未增加自动判险阈值或演示事件，空列表不代表气象安全。
