...
const bar=document.createElement('div');bar.className='mqtt-bar';bar.innerHTML='<span id="mqtt-summary">未启动 MQTT 任务</span><span id="mqtt-error" role="alert"></span><button id="pause-mqtt" data-action="pause-mqtt" hidden>暂停发送</button>';
document.querySelector('#scene-tools-menu').innerHTML='<button data-action="load-demo">载入演示样本</button><button data-action="restore-draft">恢复载入前草稿</button><button data-action="export-scene">导出场景</button><button data-action="import-scene">导入场景</button><input id="scene-file" type="file" accept="application/json,.json" hidden>';
