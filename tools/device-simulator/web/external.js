...
async function call(path, body, signal){const writing=body!==undefined&&!(path==='request'&&body.method==='GET');let response;try{response=await fetch('/api/external/'+path,{method:body===undefined?'GET':'POST',headers:body===undefined?{}:{'Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body),signal});}catch(error){if(error.name==='AbortError')throw error;throw Error(writing?'本机模拟器服务不可用；本次提交结果未知，请恢复后刷新记录核对。':'本机模拟器服务不可用，请恢复后刷新记录。');}let data;try{data=await response.json();}catch{throw Error(writing?'系统响应无法读取；提交结果未知，请刷新记录核对。':'系统响应无法读取，请刷新重试。');}if(response.status===401){requestSerial++;setConnection({connected:false});context=null;inbox=null;message('系统登录已失效，请重新登录；当前报文草稿已保留。',true);render();throw Error('系统登录已失效，请重新登录；当前报文草稿已保留。');}if(!response.ok)throw Error(data.error||'系统请求失败');return data;}
...
$('#refresh').addEventListener('click',()=>{message('');refresh();});document.querySelectorAll('[data-tab]').forEach(button=>button.addEventListener('click',()=>setTab(button.dataset.tab)));
