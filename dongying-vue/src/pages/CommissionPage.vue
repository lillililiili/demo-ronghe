...
function connectTask() { runAction(async () => { active.value = await commissionApi.connect(active.value.commission_id, active.value.version); }, isSimulation.value ? '正在模拟设备连接' : '正在建立连接'); }
function saveConfig() {
  if (!config.host?.trim() || !config.port) { error.value = '主机和端口为必填'; return; }
  runAction(async () => { active.value = await commissionApi.configure(active.value.commission_id, { version: active.value.version, ...config }); }, '配置已保存，可以开始调测');
}
  ...
  <div class="view commission-page" id="view" :aria-busy="loading || actionBusy">
    ...
    <div class="commission-main">
      ...
      <UPanel title="调测结论"  nopad>
        <div v-if="report" class="report-card"><NTag :type="statusType(report.status)" :bordered="false" size="large">{{ statusText(report.status, report.simulated) }}</NTag><p>{{ report.warning }}</p><div v-for="item in report.results?.items || []" :key="item.code" class="result-item"><span><b>{{ item.label || item.code }}</b><small>{{ item.basis || (report.simulated ? '演示检查标准' : report.protocol_code) }}</small></span><NTag size="small" :type="item.result==='FAILED'?'error':item.result==='UNTESTABLE'?'warning':'success'" :bordered="false">{{ item.result === 'PASSED' ? '通过' : item.result === 'FAILED' ? '失败' : item.result === 'UNTESTABLE' ? '不可判定' : item.result }}</NTag></div><NButton type="primary" @click="showReport">查看完整 JSON</NButton></div><NEmpty v-else description="任务完成后可在这里查看报告" class="empty-block" />
      </UPanel>
    </div>
  </div>
</template>
