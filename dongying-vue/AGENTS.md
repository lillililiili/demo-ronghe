...
- 无人机流程的后续设计遵循根目录“自动研判与快速响应”原则，人工核实不是所有事件的长期必经设计。当前实现仍有人工核实门槛，并已接入联系劝离、现场观察、条件反制申请和独立处罚移送，见 `../docs/backend-stage19/uav-advisory-workflow.md`；不得把较早的五步演示链强加为所有目标的统一流程，也不得把尚未落地的自动判定伪装成已实现。多个入口必须复用同一后端事件及共享动作，并显示一致进度。
