...
const statusText = value => ({ ONLINE: '在线', OFFLINE: '离线', ABNORMAL: '异常', UNKNOWN: '未知', CONNECTED: '已连接', DISCONNECTED: '未连接', CONNECTING: '连接中', DEGRADED: '链路降级' })[value] || '未知';
/* 指标编码与开关量都是协议里的英文名，上屏说人话。 */
const METRIC_LABEL = { link_latency_ms: '链路时延', packet_loss_pct: '丢包率', packet_loss_rate: '丢包率', signal_dbm: '信号强度', signal_strength_dbm: '信号强度', relay_state: '继电器状态字', temperature_c: '温度', cpu_pct: '处理器占用', memory_pct: '内存占用' };
...
/* 指标来源只给内部适配器名；屏幕上说人话，认不出的就说“来源未声明”，不把英文名漏出去。 */
const METRIC_SOURCE_TEXT = { 'mock-adapter': '模拟适配器', 'device-report': '设备上报', 'radar-direct': '雷达直连', 'mqtt': '消息接入' };
function metricSourceText(source) { if (!source) return '来源未声明'; return METRIC_SOURCE_TEXT[source] || (/[\u4e00-\u9fff]/.test(source) ? source : '来源未声明'); }
