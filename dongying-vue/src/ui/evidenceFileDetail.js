...
import { EVIDENCE_CUSTODY_LABEL, EVIDENCE_CUSTODY_TAG, EVIDENCE_SUBJECT_LABEL, labelOf, readableNo } from '@/ui/labels.js';
/* 只有在库文件能下载：其余状态按钮禁用并说明（决策 15-58）。 */
const DOWNLOAD_BLOCKED = { PENDING: '文件还在入库中，暂不能下载', MISSING: '文件缺失，不能下载', CORRUPT: '文件内容与入库时不一致，不能下载' };
...
export function fmtEvidenceTime(ms) {
  ...
  const parts = Object.fromEntries(new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23' }).formatToParts(d).map(p => [p.type, p.value]));
...
export function renderEvidenceFileDetail(f, options = {}) {
  ...
  const custodyShowsHold = f.status !== 'DESTROYED' && f.held && (!f.custody || f.custody === 'HELD');
  ...
  const actions = mode === 'preview' ? '' : f.status === 'DESTROYED'
    ...
    ${canDestroy
      ...
      : ''}`;
  return `  ${U.sect('保管', U.kv([
    ...(f.status !== 'DESTROYED' ? [['保管结论', custodyTag(f)]] : []),
    custodyShowsHold
      ? ['冻结原因', esc(activeHold?.reason || '未记录')]
